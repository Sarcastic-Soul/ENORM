...
async function runCloudOnly(batches) {
    ...
    for (const batch of batches) {
        ...
        console.log(
            `[Batch ${batch.id}] Cloud Dist: ${distToCloud.toFixed(0)}`,
        );
        ...
        for (let i = 0; i < BATCH_SIZE; i++) {
            try {
                ...
                const computeTime = parseFloat(
                    res.data.duration_ms || totalTime,
                );
                const networkDelay =
                    totalTime - computeTime > 0 ? totalTime - computeTime : 0;
            ...
            try {
                ...
                if (e.response && e.response.status === 503) {
                    try {
                        ...
                        const totalTime = Date.now() - cloudStart;
                        const computeTime = parseFloat(
                            cloudRes.data.duration_ms || totalTime,
                        );
                        const cloudDelay =
                            totalTime - computeTime > 0
